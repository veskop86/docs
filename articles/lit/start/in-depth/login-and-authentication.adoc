...
Early server calls for state initialization should be delayed until authentication succeed, thus in the `crm-store.ts` modify the `constructor` method to delay the call to `initFromServer` method, and add a check to this method to avoid unauthenticated requests.
